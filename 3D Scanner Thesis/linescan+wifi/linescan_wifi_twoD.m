            ...
            elseif strcmp(input_axis_2D, 'xz') 
             ...
             data_generator('1_1');
               for i= 1:revol_a
                    ...
                    motor_mover_line(step_size_b,b_axis,step_amount_1D);
                    for j= 2:(revol+1)
...
if (step_size_1D > 0) 
    c = 1;
    intermediate  = fprintf(tcpClient,'%d,%d,%d', a, step_size_1D, c);
 else
    % Pad the step_size with zeros
    c = 0;
    intermediate = sfprintf(tcpClient,'%d,%d,%d', a, abs(step_size_1D), c);
 end
