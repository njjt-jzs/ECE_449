...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    step_amount = input.substring(thirdComma + 1).toInt();
    ...
    if (axis == 1) {
      for(int j = 0 ; j < step_amount; j++) {
        move_x (1,1, step_size,dir);
        delay(1000);
      }
    }
    if (axis == 2){
      move_y (1,1, step_size,dir);    
    }
    if (axis == 3){
      move_z(1,1, step_size,dir);
    }
    ...
    axis = input.substring(firstComma + 1, secondComma).toInt();
    step_size = input.substring(secondComma + 1).toInt();
...
void move_x (int speed, int step_size, int d,char dir){ // the speed is in milisecond/revolution
// dir = 1/0
if (dir == '1'){
...
void move_y (int speed, int step_size, int d,char dir){ // the speed is in milisecond/revolution
// dir = 1/0
if (dir == '1'){
...
void move_z (int speed, int step_size, int d,char dir){ // the speed is in milisecond/revolution
// dir = 1/0
if (dir == '1'){
