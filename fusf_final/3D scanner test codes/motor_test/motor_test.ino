...
void loop() {
  if (Serial.available() > 0) {
    ...
    char y_dir_info = receivedString.charAt(10);
    ...
    if (distance_x != 0) {
      move_x (1,1, distance_x,x_dir_info);

    }
    if (distance_y != 0){
      move_y (1,1, distance_y,y_dir_info);    
    }
    if (distance_z != 0){
      move_z(1,1, distance_z,z_dir_info);
    }
    ...
    delay(4);
    //Serial.println("got\n");
    Serial.println(y_dir_info);
  } else{
    
  }
}

void move_x (int speed, int step_size, int d,int dir){ // the speed is in milisecond/revolution
// dir = 1/0
if (dir == '1'){
...
void move_y (int speed, int step_size, int d,int dir){ // the speed is in milisecond/revolution
// dir = 1/0
if (dir == '1'){
...
void move_z (int speed, int step_size, int d,int dir){ // the speed is in milisecond/revolution
// dir = 1/0
if (dir == '1'){
